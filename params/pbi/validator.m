...
shared Product_EN = let
    ...
    Custom1 = Table.AddColumn(#"Reordered Columns2", "COORDINATE", each "1."&[ThemeMemberId]&"."&[SexId]),
    Custom2 = Table.RemoveColumns(#"Custom1",{"Sex_Dimension", "ThemeMemberId", "SexId"}),
    Custom3 = Table.ReorderColumns(#"Custom2",{"REF_DATE", "GEO", "DGUID", "Member", "Sex", "UOM", "UOM_ID", "SCALAR_FACTOR", "SCALAR_ID", "VECTOR", "COORDINATE", "Value", "STATUS", "SYMBOL", "TERMINATED", "DECIMALS"})
in
    Custom3;
...
shared Theme_to_Process = 0 meta [IsParameterQuery=true, Type="Any", IsParameterQueryRequired=true];

shared Query1 = let
    Source = Sql.Database("SQLB-DB-D05", "STC_DV", [Query="SELECT * FROM gis.IndicatorValues WHERE IndicatorValueId in (Select IndicatorValueId FROM gis.GeographyReferenceForIndicator WHERE IndicatorId in (Select IndicatorId FROM gis.Indicator WHERE IndicatorThemeId = 984010001))"])
in
    Source;